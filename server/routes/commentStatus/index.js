...
router.post("/update", async (req, res) => {
  try {
    ...
    if (isLogin) {
      ...
      const targetCommentDoc = await CommentsModel.findOne({
        _id: id,
      });
      if (!targetCommentDoc) {
        res.send({
      ...
      const doc = await CommentStatusModel.findOne({
        creator: userId,
        target: id,
        work: workId,
      });
      if (doc) {
        // 已经操作过
        await CommentStatusModel.findByIdAndUpdate(
          { _id: doc._id },
          {
        ...
        // 没有操作过
        await CommentStatusModel.create({
...
router.get("/getAllStatus", async (req, res) => {
  try {
    ...
    const list = await CommentStatusModel.find(_filter);
    res.send({
      code: 0,
      message: "获取成功",
      data: list,
...
router.get("/getStatusById", async (req, res) => {
  try {
    const { id, } = req.query;
    const likeCount = await CommentStatusModel.count({
      target: id,
      status: 1,
    });
    const disLikeCount = await CommentStatusModel.count({
      target: id,
      status: 2,
    });
    const data = {
